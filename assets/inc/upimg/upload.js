...
function imgUploader(box, maximg, subbutton) {
    ...
    if (arguments.length === 1 || maximg == 10 || maximg == 11 || maximg == 12) {
        this.subbutton = box;
        maximg && maximg == 10 ? this.maximg = 10 : maximg == 11 ? this.maximg = 11 : maximg == 12 ? this.maximg = 12 : this.maximg = 1;
    } else {
...
//          Comm.PopUpKeyboard({code:2},function(d){
//          	if(d==1){
	console.log(me.box)
	setTimeout(function(){
		var obj = {
                url: config.root + '/api/imgupload/app/getUploadToken?_token=' + Comm.db("_token") + '&_device=' + _d_,
                remain: me.box ? (me.maximg - me.imgList.length) : (me.maximg == 10 ? 10 : (me.maximg == 11 ? 11 : (me.maximg == 12 ? 12 : 1)))
            };
...
imgUploader.prototype.addImgItem = function(src, del) {
    ...
    ibox.className = "box";
    ibox.innerHTML = '<img onerror="this.src=\'' + this._errorSrc + '\';this.onerror=null" class="upimg" src="' + Comm.OSS.getImgUrl(src, this._errorSrc) + '">' + (del ? '<div class="upimg-del" onclick="imgUploader[\'' + this.subbutton + '\'].del(' + (this.imgList.length - 1) + ')"></div>' : '');
...
imgUploader.prototype.del = function(i) {
    ...
    me.constructor.imgupimg.imgList.splice(i, 1);
    console.log(me.constructor.imgupimg.imgList.length)
    me.delimgs(i, 1);
    //          }
    //      }
...
imgUploader.prototype.fill = function(del) {
    ...
    if (this.box) {
        ...
        if (cimg) cimg.src = Comm.OSS.getImgUrl(cimgUrl);
    }
